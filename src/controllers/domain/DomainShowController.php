...
class DomainShowController{
    ...
    public function get(){
        ...
        $is_reference = $this->domain_model->getTagReferenceId(intval($_GET["id"]), $this->user_id);

        if($is_reference){
            $this->domain_id = $this->domain_model->getDomainID($is_reference, $this->user_id);
        }else{
            $this->domain_id = $_GET["id"];
        }

        $specificTagsData               = $this->tag_modal->getSpecificTagsData($this->user_id, intval($this->domain_id));
        $specificTagsDataWithCodeRange  = $this->tagRange_modal->getTagDataWithCodeRange($this->user_id, intval($this->domain_id));
        $copySite                       = $this->domain_model->getCopyDomainData($this->user_id, intval($_GET["id"]), "copy");
        $directorySite                  = $this->domain_model->getCopyDomainData($this->user_id, intval($_GET["id"]), "directory");
        $specificDomainData             = $this->domain_model->getSpecificDomainData($this->user_id, intval($_GET["id"]));
        $parent_domain = $this->domain_model->getParentDomainData($this->user_id, $specificDomainData["parent_domain_id"]);
        $original_domain = $this->domain_model->getOriginalDomainData($this->user_id, $specificDomainData["original_parent_id"]);
        $copySiteAll                    = $this->domain_model->getAllCopyDomainData($this->user_id, intval($_GET["id"]), "copy");
